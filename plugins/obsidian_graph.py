...
class ObsidianInteractiveGraphPlugin(BasePlugin):
    ...
    def collect_pages(self, nav: MkDocsNav, config: MkDocsConfig):
        for page in nav.pages:
            ...
            node_path = self.get_page_path(page)

            category = "Genel"
            if "/Aileler/" in node_path: category = "Aile"
            elif "/Eşyalar/" in node_path: category = "Eşya"
            elif "/Hizipler/" in node_path: category = "Hizip"
            elif "/Kişiler/" in node_path: category = "Kişi"
            elif "/Maceralar/" in node_path: category = "Macera"
            elif "/Mekânlar/" in node_path: category = "Mekan"
            elif "/Yaratık Külliyatı/" in node_path: category = "Yaratık"
...
def find_best_target(nodes, wikilink: str) -> str:
    # Remove any Obsidian aliases like [[PageName|Alias]]
    clean_link = wikilink.split('|')[0]
    
    target_page_path = ""
    # This regex ensures we match the EXACT filename, 
    # allowing for an optional .md extension.
    # It looks for: (Start of string OR a slash) + Exact Link + (End of string OR .md)
    pattern = re.compile(r"(^|/)" + re.escape(clean_link) + r"(\.md)?$", re.IGNORECASE)

    for k in nodes.keys():
        if pattern.search(k):
            # If we find an exact match, we return it immediately.
            # You can still keep the 'abslen' logic if you want the "shallowest" 
            # path, but exact matching usually solves the false-link issue.
            return k
